...
export default function CreateWizard() {
  ...
  if (phase === "caste") {
    return (
      <div className="min-h-dvh bg-[#f4f1ea] px-6 py-10">
    ...
    return (
      <div className="flex h-dvh flex-col bg-[#dbe6ef]">
        ...
        <div className="shrink-0 bg-white p-4 pb-[max(1rem,env(safe-area-inset-bottom))] text-center shadow-[0_-8px_24px_rgba(20,30,60,0.1)]">
          <button onClick={selectChosen} className="rounded-lg bg-[#2b3a67] px-8 py-3 text-sm font-medium text-white hover:bg-[#23315a]">
            Select this design →
          </button>
        </div>
      </div>
              ...
              {isFinish ? (
                user ? (
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => save(true)} disabled={busy} className="rounded-lg bg-[#2b3a67] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#23315a] disabled:opacity-60">
                      {busy ? "…" : "Publish"}
                    </button>
                  </div>
                ) : (
                ...
                <button
                  ...
                  className="rounded-lg bg-[#2b3a67] px-8 py-2.5 text-sm font-medium text-white hover:bg-[#23315a]"
                >
                  Next →
                </button>
              )}
