...
export default function DashboardPage() {
  ...
  if (loading || !user) {
    return (
      <div
        className="flex h-screen items-center justify-center bg-[var(--b-bg)] text-[rgba(43,27,18,0.55)]"
  ...
  return (
    <div className="brand-page min-h-screen">
      ...
      <header className="flex items-center justify-between border-b border-[rgba(43,27,18,0.08)] bg-white px-6 py-4">
        <div className="flex items-center gap-5" style={{ fontFamily: "var(--f-body)" }}>
          ...
          <Link href="/gallery" className="text-sm text-[rgba(43,27,18,0.7)] hover:text-[var(--b-ink)]">Designs</Link>
          {user.role === "admin" ? (
            <Link href="/admin" className="text-sm text-[rgba(43,27,18,0.7)] hover:text-[var(--b-ink)]">Admin</Link>
          ) : null}
        </div>
        <div className="flex items-center gap-4" style={{ fontFamily: "var(--f-body)" }}>
          <span className="text-sm text-[rgba(43,27,18,0.6)]">{user.email}</span>
          <button onClick={logout} className="text-sm text-[var(--b-primary-deep)] hover:underline">Log out</button>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-4xl px-6 py-10">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-medium italic text-[var(--b-ink)] sm:text-4xl" style={{ fontFamily: "var(--f-serif)" }}>
              Your celebrations
            </h1>
          </div>
          <Link
            href="/create"
            className="rounded-full bg-[var(--b-primary)] px-5 py-2.5 text-sm font-medium text-white shadow-[0_10px_24px_rgba(43,27,18,0.3)] transition-colors hover:bg-[var(--b-primary-deep)]"
            style={{ fontFamily: "var(--f-body)" }}
          >
            + New invitation
          </Link>
        ...
        {items === null ? (
          <p className="mt-8 text-[rgba(43,27,18,0.5)]" style={{ fontFamily: "var(--f-body)" }}>Loading…</p>
        ) : items.length === 0 ? (
          <div className="mt-10 rounded-2xl border border-dashed border-[rgba(43,27,18,0.3)] bg-white/60 p-10 text-center">
            <p className="text-lg italic text-[rgba(43,27,18,0.65)]" style={{ fontFamily: "var(--f-serif)" }}>
              No invitations yet.
            </p>
            <Link href="/gallery" className="mt-3 inline-block text-sm font-medium text-[var(--b-gold)] hover:underline" style={{ fontFamily: "var(--f-body)" }}>
          ...
          <div className="mt-6 space-y-3" style={{ fontFamily: "var(--f-body)" }}>
            {items.map((inv) => (
              <div key={inv.id} className="rounded-xl border border-[rgba(43,27,18,0.15)] bg-white p-5 shadow-[0_10px_30px_rgba(43,27,18,0.05)]">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="text-[17px] font-medium italic text-[var(--b-ink)]" style={{ fontFamily: "var(--f-serif)" }}>
                      {inv.names || "Untitled"}
                    </p>
                    <p className="text-xs text-[rgba(43,27,18,0.55)]">
                      <span
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    <Link href={`/studio?id=${inv.id}`} className="rounded-full border border-[rgba(43,27,18,0.2)] px-3 py-1.5 text-[var(--b-ink)] hover:bg-[var(--b-tint)]">Edit</Link>
                    {user.canDuplicate ? (
                      <button
                    ...
                    {inv.status === "published" && inv.slug ? (
                      <a href={`/i/${inv.slug}`} target="_blank" rel="noreferrer" className="rounded-full border border-[rgba(43,27,18,0.2)] px-3 py-1.5 text-[var(--b-ink)] hover:bg-[var(--b-tint)]">View live ↗</a>
                    ) : null}
                    {inv.status === "published" ? (
                    ...
                    <button onClick={() => viewRsvps(inv.id)} className="rounded-full border border-[rgba(43,27,18,0.2)] px-3 py-1.5 text-[var(--b-ink)] hover:bg-[var(--b-tint)]">RSVPs</button>
                    <button onClick={() => downloadExcel(inv)} className="rounded-full border border-[rgba(43,27,18,0.2)] px-3 py-1.5 text-[var(--b-ink)] hover:bg-[var(--b-tint)]">Excel ↓</button>
                    <button onClick={() => emailExport(inv)} className="rounded-full border border-[rgba(43,27,18,0.2)] px-3 py-1.5 text-[var(--b-ink)] hover:bg-[var(--b-tint)]">Email me</button>
                    <button
                ...
                {inv.status === "published" && inv.slug ? (
                  <p className="mt-2 text-xs text-[rgba(43,27,18,0.45)]">
                    {PUBLIC_ORIGIN}/i/{inv.slug}
                  </p>
                ) : null}

                {rsvpFor === inv.id ? (
                  <div className="mt-3 rounded-lg bg-[var(--b-tint)] p-3 text-sm">
                    {!rsvpData ? (
                      <p className="text-[rgba(43,27,18,0.5)]">Loading RSVPs…</p>
                    ) : rsvpData.total === 0 ? (
                      <p className="text-[rgba(43,27,18,0.6)]">No RSVPs yet.</p>
                    ) : (
                      <>
        ...
        <p className="mt-6 text-[11px] text-[rgba(43,27,18,0.4)]" style={{ fontFamily: "var(--f-body)" }}>API: {API_BASE}</p>
      </main>
    </div>
  );
}
