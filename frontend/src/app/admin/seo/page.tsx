...
function ScoreDot({ score }: { score?: number | null }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-xs text-[var(--b-muted)]">
    ...
    <div className="min-h-screen bg-[#f4f1ea]">
      ...
      <main className="mx-auto max-w-4xl px-6 py-10">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="font-display text-2xl uppercase tracking-[0.1em] text-[var(--b-ink)]">AI SEO</h1>
          ...
          {status ? (
            status.configured ? (
              <div className="flex items-center gap-3">
                ...
                <button onClick={runAudit} disabled={busy} className="rounded-lg bg-[var(--b-primary)] px-4 py-2 text-sm font-medium text-white hover:bg-[#23315a] disabled:opacity-60">
                  {busy ? "…" : "Run audit now"}
                </button>
              </div>
            ) : (
        ...
        {msg ? <p className="mb-4 rounded-lg bg-white px-4 py-2 text-sm text-[var(--b-body)] ring-1 ring-[var(--b-border)]">{msg}</p> : null}
        ...
        {/* AI blog drafting */}
        <section className="mb-8 rounded-xl border border-[var(--b-border)] bg-white p-5">
          <h2 className="text-sm font-semibold uppercase tracking-[0.12em] text-[var(--b-muted)]">Write a blog post with AI</h2>
          <p className="mt-1 text-xs text-[var(--b-muted)]">Give a topic; Claude drafts a full SEO-friendly post and opens it in the editor for you to review.</p>
          <div className="mt-3 flex flex-col gap-2 sm:flex-row">
            <input
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
              placeholder="e.g. 10 wording ideas for a Christian wedding invitation"
              className="flex-1 rounded-lg border border-[var(--b-border)] px-3 py-2 text-sm outline-none focus:border-[var(--b-muted)]"
            />
            <button
              onClick={generateDraft}
              disabled={drafting || !status?.configured || !topic.trim()}
              className="rounded-lg bg-[var(--b-primary)] px-4 py-2 text-sm font-medium text-white hover:bg-[#23315a] disabled:opacity-50"
            >
        ...
        {insights && insights.pages.length ? (
          <section className="mb-8 rounded-xl border border-[var(--b-border)] bg-white p-5">
            <div className="mb-1 flex flex-wrap items-center justify-between gap-2">
              <h2 className="text-sm font-semibold uppercase tracking-[0.12em] text-[var(--b-muted)]">Pages by traffic</h2>
            ...
            <p className="mb-3 text-xs text-[var(--b-muted)]">Lowest-traffic first — the weekly audit optimises these before the rest.</p>
            <div className="divide-y divide-[var(--b-tint)]">
              {insights.pages.slice(0, 8).map((p) => (
                <div key={p.contentId} className="flex items-center justify-between gap-3 py-2">
                  <div className="min-w-0">
                    <Link href={`/admin/content/${p.contentId}`} className="truncate text-sm font-medium text-[var(--b-ink)] hover:text-[var(--b-ink)] hover:underline">
                      {p.title}
                    </Link>
                    <span className="ml-2 text-xs text-[var(--b-muted)]">{p.path}</span>
                  </div>
                  <div className="flex shrink-0 items-center gap-2">
                    ...
                    <span className="w-20 text-right text-sm text-[var(--b-body)]">{p.views.toLocaleString()} views</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* review queue */}
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-[0.12em] text-[var(--b-muted)]">
        ...
        {proposals === null ? (
          ...
          <p className="rounded-xl border border-dashed border-[var(--b-border-soft)] p-8 text-center text-[var(--b-muted)]">
            No pending suggestions. {status?.configured ? "Run an audit to generate some." : ""}
          </p>
        ) : (
          <div className="space-y-4">
            {proposals.map((p) => (
              <div key={p.id} className="rounded-xl border border-[var(--b-border)] bg-white p-5">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <span className="rounded-full bg-[var(--b-tint)] px-2 py-0.5 text-[10px] uppercase tracking-wide text-[var(--b-muted)]">{p.content.type}</span>
                    <span className="font-medium text-[var(--b-ink)]">{p.content.title}</span>
                    <span className="text-xs text-[var(--b-muted)]">/{p.content.type === "post" ? "blog" : "p"}/{p.content.slug}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    {p.traffic ? (
                      <span className="text-xs text-[var(--b-muted)]">{p.traffic.views.toLocaleString()} views · {p.traffic.days}d</span>
                    ) : null}
                    <ScoreDot score={p.score} />
                    <span className="rounded-full bg-[var(--b-bg)] px-2 py-0.5 text-[10px] text-[var(--b-muted)]">{p.source}</span>
                  </div>
                </div>
                ...
                {p.issues.length ? (
                  <div className="mt-3">
                    <p className="text-[11px] font-medium uppercase tracking-wide text-[var(--b-muted)]">Issues found</p>
                    <ul className="mt-1 list-disc pl-5 text-xs text-[var(--b-body)]">
                      {p.issues.map((it, i) => <li key={i}>{it}</li>)}
                    </ul>
                  </div>
                ) : null}
                {p.rationale ? <p className="mt-2 text-xs italic text-[var(--b-muted)]">{p.rationale}</p> : null}

                <div className="mt-4 flex justify-end gap-2">
                  <button onClick={() => decide(p, false)} className="rounded-lg border border-[var(--b-border-soft)] px-4 py-1.5 text-xs text-[var(--b-body)] hover:bg-[var(--b-bg)]">Dismiss</button>
...
function DiffCol({ label, title, desc, muted }: { label: string; title?: string | null; desc?: string | null; muted?: boolean }) {
  return (
    <div className={`rounded-lg border p-3 ${muted ? "border-[var(--b-border)] bg-[var(--b-bg)]/60" : "border-emerald-200 bg-emerald-50/40"}`}>
      <p className="text-[10px] font-semibold uppercase tracking-wide text-[var(--b-muted)]">{label}</p>
      <p className={`mt-1 text-sm font-medium ${muted ? "text-[var(--b-muted)]" : "text-[var(--b-ink)]"}`}>{title || <span className="text-[var(--b-border-soft)]">— none —</span>}</p>
      <p className={`mt-1 text-xs ${muted ? "text-[var(--b-muted)]" : "text-[var(--b-body)]"}`}>{desc || <span className="text-[var(--b-border-soft)]">— none —</span>}</p>
    </div>
  );
}
